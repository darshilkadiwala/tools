...
const updateEMIDateSchema = z.object({
  ...
  (data) => data.endEMINumber >= data.startEMINumber,
  {
...
export function UpdateEMIDateDialog({
  ...
  const handleSubmit = async (data: UpdateEMIDateFormValues) => {
    try {
      ...
      const allEMIs = await db.emiSchedules
        .where('loanId')
        .equals(loanId)
        .sortBy('emiNumber')
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Update the due dates for a range of EMIs. The first EMI in the range will be set to the new start date, and subsequent EMIs will be calculated monthly from that date.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
